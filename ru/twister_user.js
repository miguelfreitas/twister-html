...
function loadWalletlUsers(cbFunc, cbArg) {
    twisterRpc("listwalletusers", [],
               ...
               function(args, ret) {
                   alert("Ошибка подключения к локальному демону twisterd.");
               }, {});
}
...
function checkUsernameAvailability() {
    ...
    if( username.length > 16 ) {
        $availField.text("Должно быть не более 16 знаков, или менее.");
        return;
    }

    //Check for non-alphabetic characters and space
    if(username.search(/[^a-z0-9_]/) != -1) {
        $availField.text("Только английские буквы и цифры.");
        return;
    }

    $availField.text("Проверяю...");

    dumpPubkey(username, function(dummy, pubkey) {
                            var notAvailable =  pubkey.length > 0
                            var $availField = $(".availability");
                            if( notAvailable ) {
                                $availField.text("Недоступно");
                            } else {
                                $availField.text("Ура, доступно!");
