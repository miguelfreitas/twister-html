...
function checkNetworkStatusAndAskRedirect(cbFunc, cbArg) {
    networkUpdate(function(args) {
                      if( !twisterdConnectedAndUptodate ) {
                          var redirect =
                              window.confirm("Фоновый процесс twisterd не подключен к сети или\n" +
                                             "цепочка блоков устарела. Если вы будите находиться на этой странице\n" +
                                             "ваши действия могут быть не выполнены.\n" +
                                             "Хотите перейти на страницу Настройки сети?");
                          if( redirect )
                              $.MAL.goNetwork();
                      } else {
                        if( args.cbFunc )
...
function timeSincePost(t) {
    ...
    var expression = "";
    if(t_delta < 2) {
        expression = "1 секунда"
    }
    else if(t_delta < 60) {
        expression = t_delta + " секунд"
    }
    else if(t_delta < 120) {
        expression = "1 секунда"    
    }
    else if(t_delta < 60 * 60) {
        expression = Math.floor(t_delta/60) + " минут"
    }
    else if(t_delta < 2 * 60 * 60) {
        expression = "1 час"
    }
    else if(t_delta < 24 * 60 * 60) {
        expression = Math.floor(t_delta/60/60) + " часов"
    }
    else if(t_delta < 2 * 24 * 60 * 60) {
        expression = "1 день"
    }
    else {
        expression = Math.floor(t_delta/24/60/60) + " дней"
    }
    return expression + " назад";
}
...
function openProfileModal(e)
{
    ...
    //título do modal
    $( "."+profileModalClass + " h3" ).text( username + "'а Профиль" );
}
...
var reTwistPopup = function( e )
{
    ...
    //título do modal
    $( ".reTwist h3" ).text( "Перепостить эту запись вашим подписчикам ?" );
...
var postSubmit = function(e)
{
    ...
    $replyText.val("");
    $replyText.attr("placeholder", "Ваше сообщение было отправлено!");
    closeModal($this);
}
